...
Процедура ОбработкаВозможна()
	...
	Если ЧитатьЖурнал Тогда
		...
		Ожидаем.Что(ПутьКВнешнейКомпонентеSqlLite).Заполнено();
		Ожидаем.Что(ПутьКSqlLite).Заполнено();
		Ожидаем.Что(СтрокаСоединенияСSQL).Заполнено();
		Ожидаем.Что(ИмяИнформационнойБазы).Заполнено();
		Ожидаем.Что(ПредставлениеИнформационнойБазы).Заполнено();
		Ожидаем.Что(ОписаниеИнформационнойБазы).Заполнено();	
		Ожидаем.Что(ЧислоСтрокДляЗаписи).Заполнено();
		Ожидаем.Что(ПаузаСекунд).Заполнено();
