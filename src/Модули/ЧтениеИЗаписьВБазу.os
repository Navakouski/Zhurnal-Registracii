...
Процедура ПрочитатьСобытия(ОбработкаЖурнала)
	...
	Для Каждого ТекСтрока Из ТаблицаПоследнихIDSqlLite Цикл
		...
		Если ВыводитьЛог Тогда
			...
			Если ИмяТаблицыSQLLite = "EventLogMetadata" Тогда
				ТекстСообщения = СтрШаблон("Начата обработка таблицы %1. Обрабатываются записи с %2 по %3. КоличествоНовыхЗаписей %4. Реальное количество новых записей отличается", ИмяТаблицыSQL, IDпоследнейЗаписиВSQL, IDпоследнейЗаписиВSQLLite, КоличествоНовыхЗаписей);
			Иначе
...
Функция IDПоследнейЗаписиВEventLogMetadataSqlLite(ОбработкаЖурнала)
...
Функция IDПоследнейЗаписиВСоответствияСтрокМетаданнымSql(ОбработкаЖурнала)
	...
	ЗапросSQL.Текст = "Select TOP (1) eventLogID From СоответствияСтрокМетаданным where ИнформационнаяСистема = @ИнформационнаяСистема ORDER BY eventLogID DESC";
	ЗапросSQL.УстановитьПараметр("ИнформационнаяСистема", ИнформационнаяСистема);
	РезSql = ЗапросSQL.Выполнить();
	
	Если РезSql.Пустой() Тогда
...
Функция СоответствиеТаблиц()
	...
	// Соответствия строк метаданным
	
	ДанныеТаблицыСоответствия = Новый Структура();
	ДанныеТаблицыСоответствия.Вставить("ИмяТаблицы", "СоответствияСтрокМетаданным");
	СтруктураТаблицыСоответствияСтрокМетаданным = СтруктураТаблицыСоответствияСтрокМетаданным();
	ДанныеТаблицыСоответствия.Вставить("СоответствиеКолонок", СтруктураТаблицыСоответствияСтрокМетаданным);
	ДанныеТаблицыСоответствия.Вставить("ИмяКолонкиАйди", "eventLogID");
	СоответствиеТаблиц.Вставить("EventLogMetadata", ДанныеТаблицыСоответствия);
...
Функция СтруктураТаблицыСоответствияСтрокМетаданным()
	
	СтруктураТаблицы = Новый Структура();
	СтруктураТаблицы.Вставить("eventLogID", "eventLogID");
	СтруктураТаблицы.Вставить("metadataCode", "metadataCode");
