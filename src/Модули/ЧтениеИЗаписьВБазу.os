...
// Состояние транзакции не завершена не подвержден, нужно проверить индекс
...
Процедура ПрочитатьСобытия(ОбработкаЖурнала)
	...
	Для Каждого ТекСтрока Из ТаблицаПоследнихIDSqlLite Цикл
		...
		Ожидаем.Что(ПодходящиеСтроки.Количество()).Равно(1);
...
Функция ДатаФорматаSQL(ДатаSQLLite)
	...
	Если ЗначениеЗаполнено(ДатаSQLLite) Тогда
		Дата1с = '00010101000000' + ДатаSQLLite / 10000;
...
Процедура ЗаполнитьСостоянияТранзакции(ОбработкаЖурнала)
	...
	ЗапросВставка.УстановитьСоединение(Соединение);
	ИнформационнаяСистема = ОбработкаЖурнала.ПолучитьИнформационнаяСистема();
	ЗапросВставка.Текст = "insert into СостоянияТранзакции (ИнформационнаяСистема, Id, Представление, Имя) values(@ИнформационнаяСистема, 1, 'Зафиксирована', 'Committed'), (@ИнформационнаяСистема, 2, 'Отменена', 'RolledBack'), (@ИнформационнаяСистема, 3, 'Нет транзакции', 'NotApplicable'), (@ИнформационнаяСистема, 4, 'Не завершена', 'Unfinished')";
